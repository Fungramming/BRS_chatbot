...
from .StatusCodeHelper import statushelper
...
def get_orders_delivering():
    ...
    if 'error' in result:
        return jsonify({'error': result['error']}), statushelper(response.status_code)

    orders = result['orders']

    if len(orders) == 0:
        return jsonify({'orders': '배송상태에 있는 주문이 없습니다.'}), statushelper(204)
    ...
    return jsonify({'orders': orders_list}), statushelper(response.status_code)
...
def get_orders_delivered():
    ...
    return jsonify({'orders': orders_list, 'next_url': next_url, 'total_count': len(orders_list)}), statushelper(response.status_code)
